...
class Field:
    ...
    def validate(self, data):
        ...
        """
        #LoggerUtility.logDebug('Path: ' + str(self.path) + ', Field Value: ' + str(field_value) + ', Data: ' + str(data))
    ...
    def _check_conditional(self, then_part, data_field_value, data):
        ...
        if then_part:
            ...
            if data_field_value == None:
                ...
                LoggerUtility.logDebug(self.path)
        ...
        if data_field_value is None:
            ...
            if data_field_value == "":
                ...
                if self.type == TYPE_ENUM and str(data_field_value).lower() not in [x.lower() for x in self.values]:
                    ...
                    try:
                        if hasattr(self, 'upper_limit') and Decimal(str(data_field_value)) > self.upper_limit:
                        ...
                        if hasattr(self, 'lower_limit') and Decimal(str(data_field_value)) < self.lower_limit:
                        ...
                        return FieldValidationResult(False, "failure to perform decimal validation on " + str(data_field_value) + ", error: %s" % (str(e)), self.path)
                    ...
                    try:
                        ...
                        for choice in self.choices:
                            ...
                            if value is not None:
                                count += 1
                        if count == 0:
                            LoggerUtility.logDebug(self.path + '.' + choice)
...
class TestCase:
    def __init__(self, filepath=None):
        LoggerUtility.logInfo("Setting up TestCase");
        self.config = ConfigParser(interpolation=ExtendedInterpolation())
        self.record_parser = {"json": json.loads, "csv": self.parse_csv}
        if filepath is None:
            default_config = pkg_resources.resource_string(__name__, "/../../../tmp/config")
        ...
        if self.config.has_section("_settings"):
            ...
            LoggerUtility.logError("Config file for the data log does not have required '_settings' field defined.")
            raise ValidatorException("Invalid config ini file, '_settings' field not defined.")

        self.field_list = []
        self.skip_sequential_checks = set()
        LoggerUtility.logInfo("Setting up TestCase fields from data log");
        ...
        LoggerUtility.logInfo("TestCase setup complete");
    ...
    def populate_list_validations(self, keys, data, path, path_init): # Recurcive function to loop through data and populate 
        ...
        if keys[0] == 'list': #List found
            ...
            if type(data) != list: # Found list but list or elements do not exist. Add 1 entry to list to allow invalidation later
                if len(keys) != 1:
                    keys = keys[1:]
                else:
                    keys = []
                ...
                for i in (range(length)):
                    ...
                    if len(keys) != 1:
                        ...
                        LoggerUtility.logWarning('List at end of config field. Add values under list for path ' + path)
                        return
        elif keys[0] in data:
    ...
    def validate_queue(self, msg_queue):
        LoggerUtility.logInfo("Validating data log with config file...");
        results = []
        msg_list = []
        msg_count = 1
        # if header, skip over it
        if self.has_header:
            LoggerUtility.logInfo("Validating headers from CSV data log with field names from config file...");
            header = msg_queue.get()
            self.check_headers(header)

        while not msg_queue.empty():
            if msg_queue.qsize() % 100000 == 0:
                LoggerUtility.logInfo("Progress Update: " + str(msg_queue.qsize()) + " lines remaining")
        ...
        if self.SequentialValidation:
            LoggerUtility.logInfo("Sequential validation set to True");
    ...
    def parse_csv(self, line):
        ...
        for field in self.field_list:
            try:
                ...
                LoggerUtility.logError("Failed to parse CSV data log file field due to mismatch number of columns")
                raise
    ...
    def check_headers(self, headers):
        ...
        index = 0
        LoggerUtility.logInfo("CSV Headers: " + str(csv_fields));

        for field in self.field_list:
            if not str.lower(field.path) == str.lower(csv_fields[index]):
                LoggerUtility.logWarning("Warning: The data file CSV header '" + str.lower(csv_fields[index]) + "' does not match the config file field '" + str.lower(field.path) + "'");
            index += 1